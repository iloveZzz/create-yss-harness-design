...
const PACKAGE_NAME = "create-yss-harness-design";
...
function npmViewLatest(packageName, spawn) {
  ...
  if (!version) throw new Error("无法查询最新版本：npm view 未返回版本号");
  parseSemver(version);
  return version;
}

function npmGlobalPrefix(spawn) {
  const result = spawn("npm", ["prefix", "-g"], { encoding: "utf8", timeout: 15000 });
  if (spawnFailed(result)) return "";
...
function detectInstallKind(packageRoot, spawn, packageName = PACKAGE_NAME) {
  ...
  if (hasPathSegment(resolvedRoot, "_npx")) return { kind: "npx" };
  if (fs.existsSync(path.join(resolvedRoot, ".git"))) return { kind: "source" };
  ...
  if (projectRoot) return { kind: "local", cwd: projectRoot };
  return { kind: "unknown" };
}

function buildInstallPlan(install, packageName) {
  if (install.kind === "global") {
    return { command: "npm", args: ["install", "-g", `${packageName}@latest`], cwd: undefined };
  }
  if (install.kind === "local") {
    return { command: "npm", args: ["install", `${packageName}@latest`], cwd: install.cwd };
  }
  return null;
}

function formatInstallCommand(plan) {
  if (!plan.cwd) return `npm ${plan.args.join(" ")}`;
...
function printInstallAdvice(stdout, install) {
  ...
  if (install.kind === "source") {
    writeln(
      stdout,
      "检测到源码目录；请通过发布流程或 npm install -g create-yss-harness-design@latest 升级",
    );
  } else if (install.kind === "npx") {
    writeln(
      stdout,
      "当前通过 npx 运行；下次使用 npx create-yss-harness-design@latest 即可拿到最新版",
    );
  } else {
    writeln(stdout, "未能判断安装位置");
  }
  writeln(stdout, "建议：");
  writeln(stdout, "  npm install -g create-yss-harness-design@latest");
  writeln(stdout, "  npx create-yss-harness-design@latest");
}
...
function runUpdate(argv = [], deps = {}) {
  ...
  if (!packageRoot) throw new Error("update 缺少 packageRoot");
  if (!currentVersion) throw new Error("update 缺少 currentVersion");
  ...
  if (result.stdout) writeln(stdout, result.stdout);
  if (spawnFailed(result)) {
    const detail = spawnErrorDetail(result);
    throw new Error(`升级失败${detail ? `：${detail}` : ""}`);
  }
  if (result.stderr && result.stderr.trim()) writeln(stdout, result.stderr);
